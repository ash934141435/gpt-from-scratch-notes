...
| 项目 | 结果 | 证据 |
|---|---:|---|
| 新课程结构 | 通过 | `course/README.md` + 18 个章节 README，共 4019 行正文 |
...
| 主线代码导读 | 通过 | V0–V11 均有版本关系、运行输出和完整代码区块讲解；V2 的模型/loss 与生成由第 04、05 章共同闭环 |
| CPU 时间 | 通过 | 12 个版本共 14.24 秒；单个最长 3.53 秒 |
...
`qa/check.py` 以 `source_mode` 条件处理第 00 章，不会因无视频而误报；第 01–17 章必须严格使用 14 节新模板，旧独立图片区会直接失败。检查还会验证 27 张图片的上下文、图注、实际 H2/H3、证据角色、M 编号、时间范围、索引和磁盘文件，并要求每章说明与代码主线的关系、每个 V0–V11 版本同时具备运行结果解读和完整代码导读。
...
详细逐版本时间见 `cpu-benchmark.md`。本地完整套件 14.24 秒，低于约 3 分钟目标；V11 没有在自动验收中运行默认 200 步训练。
