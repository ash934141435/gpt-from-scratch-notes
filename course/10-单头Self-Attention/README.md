...
Q、K、V 都从同一个 x 出发，但沿两条不同职责的数据流汇合：

```mermaid
flowchart LR
    X["x [B,T,C]"] --> Q["Query [B,T,H]"]
    X --> K["Key [B,T,H]"]
    X --> V["Value [B,T,H]"]
    Q --> S["Q @ Kᵀ<br/>scores [B,T,T]"]
    K --> S
    S --> W["缩放 + 因果 Mask + Softmax<br/>weights [B,T,T]"]
    W --> O["weights @ V<br/>out [B,T,H]"]
    V --> O
```

上路决定“读谁”，下路提供“读到什么”。把 Value 单独画出来，可以避免把 `weights @ v` 错写成再次聚合 Key。
...
视频第一次建立 Q/K 投影时，直接在代码旁标出它们的 Shape，用来确认线性层只改变最后一轴：

![Query 和 Key 投影后的 Shape](../../05-Self-Attention/assets/crops/01-05-34-qk-shapes.png)

*图：Query 与 Key 都从 `[B,T,C]` 投影为 `[B,T,H]`（原视频 M087，01:05:34）*

这里的 T 没被线性层混合；真正的位置配对发生在随后 `q @ k.transpose(-2,-1)`，结果才出现两个 T 轴。
...
加入因果 mask 后，完整权重矩阵应同时满足“未来位置为 0”和“每行允许权重和为 1”。视频输出把这两个条件放在同一张矩阵里：

![因果 Mask 后的 Softmax 权重分布](../../05-Self-Attention/assets/crops/01-09-49-softmax-distribution.png)

*图：未来权重归零、允许位置按行归一化的运行结果（原视频 M092，01:09:49）*

矩阵不再像前缀平均那样每个允许项都相等，因为 Q/K 分数已经依赖输入内容；但右上三角仍必须严格为 0，因果规则没有改变。
