...
[完整 V4](./code/V4-prefix-average-demo.py)把第 07 章的循环前缀平均作为写法 1，再在本章加入矩阵写法和 mask+softmax 写法。它是一个隔离实验，不包含完整语言模型；目的就是排除其他模块干扰，只验证三种计算是否等价。
...
输入输出 Shape 相同，说明聚合不会增删 token 或通道。权重矩阵必须满足两件事：每行和为 1，右上角全为 0；前者表示做加权平均，后者表示没有读取未来。
...
| 代码区块 | 完整作用 | 自动检查什么 |
|---|---|---|
| 随机种子与 `x [B,T,C]` | 建立固定的两批、四位置、三通道输入 | 三种写法必须接收完全相同的数据 |
| `loop_average` 双循环 | 对每个 `(batch,time)` 明确切出 `:time+1` 后求均值 | 作为最贴近定义的参照答案 |
| `weights` 下三角矩阵 | 每行归一化后一次完成所有位置的前缀平均 | 应与循环结果 `allclose` |
| `scores`、mask、softmax | 用 0 表示允许、`-inf` 表示禁止，再归一化 | 应与矩阵平均相同，并为 Attention 预留可变分数 |
| `demo()` | 检查 Shape、三路数值、行和与上三角 | 任一性质破坏都会在打印前触发断言 |
| 主入口保护 | 直接运行才调用演示 | 导入该文件时不重复打印 |

这里用 `allclose` 而不是 `equal`，因为矩阵乘法与逐项平均的浮点运算顺序可能不同，数学等价的结果仍会有极小舍入差异。`torch.triu(..., diagonal=1)` 只取严格右上三角，再统计非零数，直接验证未来位置的权重全部为 0。
