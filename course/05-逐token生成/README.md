...
- `28:37–29:56`（M040–M041）：生成循环和可选 targets。
- `29:56–31:15`（M042–M043）：取最后位置并 softmax。
- `31:15–32:32`（M044–M045）：multinomial 抽样并沿时间轴拼接。
- `32:32–34:57`（M046–M047）：从换行 token 启动，未训练输出为乱码。

![生成时只取最后时间步](../../03-Bigram语言模型/assets/crops/00-30-20-last-time-step.png)
