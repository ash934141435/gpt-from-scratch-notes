...
```text
characters=1,115,394, vocabulary=65
...
1. 激活项目环境并运行 V0；
2. 把 V0 中的 `sample` 临时改为 `"Romeo:"`，确认编码后还能解码回来；
3. 新建一个 Shape 为 `[2,3]` 的浮点张量；
4. 打印它的 `shape`、`ndim`、`dtype`、第一行和最后一列；
5. 故意访问不存在的第 4 行，指出报错类型，并恢复代码。

验收参考：浮点类型通常显示为 `torch.float32`；第一行 Shape 是 `[3]`，最后一列 Shape 是 `[2]`；越界应触发 `IndexError`。完成后不要保留对 V0 的临时修改。

## 12. 过关标准

- 能说明终端、工作目录、虚拟环境各自是什么；
- 能独立运行 V0，并知道相对路径从当前目录开始解释；
