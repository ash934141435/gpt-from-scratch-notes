...
编码和解码必须使用同一份词表，它们组成一次可验证的往返：

```mermaid
flowchart LR
    A["原始文本 cab!"] -->|"encode：按 stoi 查表"| B["token ID [3,1,2,0]"]
    B -->|"decode：按 itos 查回"| C["还原文本 cab!"]
    C -->|"应与输入完全相同"| A
```

视频实际处理 Tiny Shakespeare 时，`set` 负责去重，`sorted` 负责固定编号顺序，最终得到 65 个字符：

![Tiny Shakespeare 的字符词表与大小](../../02-字符编码与训练数据/assets/crops/00-08-58-vocabulary.png)

*图：从训练文本得到稳定字符词表和 65 个字符的运行结果（原视频 M013，00:08:58）*

这里应核对两件事：词表只保留不同字符，并且顺序固定。课程代码读取仓库内的 `sources/ng-video-lecture/input.txt`，所以重复运行会得到相同的字符编号，不需要再次联网下载。
...
上面的短例子使用 `"cabca!"`，便于手算；[完整 V0](./code/V0-text-vocabulary.py)做的是同一件事，只是把输入换成仓库里的 Tiny Shakespeare，并增加断言和中文输出。V0 没有模型，也不会训练，它只是后续所有版本共用的“文本↔编号”起点。
...
示例词元： [18, 47, 56, 57, 58, 1, 15, 47, 58, 47, 64, 43, 52, 10, 0]
编解码还原结果： 'First Citizen:\n'
```

- 第一行验证读取的是课程指定数据，而不是另一个同名文件。
- 第二行是 `"First Citizen:\n"` 的字符编号；编号本身没有语义，只取决于排序后的词表。
- 第三行必须还原成原字符串；其中 `\n` 是换行符的可见写法，不是两个普通字符。
...
| 代码区块 | 做什么 | 为什么需要 |
|---|---|---|
| 文件说明与 `Path` 导入 | 说明版本目标，并使用跨平台路径对象 | 不把数据路径写死为某台电脑的绝对路径 |
| `TEXT_PATH` 与 `read_text` | 从脚本位置向上找到仓库根目录，再以 UTF-8 读取数据 | 无论从项目根目录还是别处启动，都读到同一份文本 |
| `chars`、`stoi`、`itos` | 去重排序并建立双向查表 | 为每个字符分配稳定且连续的 `0..64` 编号 |
| `encode`、`decode` | 分别完成字符到编号、编号到字符的转换 | 后续张量计算和最终文本显示共用这两个方向 |
| `demo()` | 编码固定样例，执行四个断言，再打印三项结果 | 断言负责发现错误，打印负责让学习者观察结果 |
| `if __name__ == "__main__"` | 只在直接运行文件时调用 `demo()` | 将来导入函数时不会自动打印或重复执行演示 |

类型标注中的 `str`、`list[int]` 和 `-> None` 用来说明输入输出约定，不会自动改变运行结果。四个断言依次检查数据长度、词表大小、往返还原和编号连续性；只要其中一个失败，程序就会立即停在真正不满足的条件上。
